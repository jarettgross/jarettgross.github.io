//=======================
//REGULAR PLANET FUNCTION
//=======================

//Generate planet using a quadsphere
function generatePlanet(planetSize = 200, tessellation = 60, minHeight = 1.0, maxHeight = 1.2) {
    var faceIndices = ['a', 'b', 'c'];

    var planetMesh = generateQuadsphere(planetSize, tessellation);

    //Find min, max values of noise to use for value mapping
    var values = getMinMaxValues(planetMesh.geometry.vertices, planetSize);
    ...
    for (var i = 0; i < planetMesh.geometry.faces.length; i++) {
        var curValue = 0;
        var face = planetMesh.geometry.faces[i];

        for (var j = 0; j < faceIndices.length; j++) {
            curValue = Math.max(curValue, planetMesh.geometry.vertices[face[faceIndices[j]]].length() / planetSize);
        }
...
//=====================
//TILE PLANET FUNCTIONS
//=====================

//Generate a planet using a subdivided icosahedron altered to create hexagonal/pentagonal tiles
function generateTilePlanet(planetSize = 200, subdivisions = 4, type = "NORMAL", minHeight = 1.0, maxHeight = 1.2) {
    subdivisions = Math.min(subdivisions, 6);
    ...
    } else if (type === "FLAT-HEIGHT") {
        return generateFlatHeightTilePlanet(planet, planetSize, minHeight, 1.5);
    }
}

//Regular sphere with hexagonal/pentagonal tiles
function generateNormalTilePlanet(planet, planetSize, minHeight, maxHeight) {
    var planetMesh = planet.mesh;
    ...
    for (var i = 0; i < planet.polygonGroups.length; i++) {
        ...
        for (var j = 0; j < faces.length; j++) {
            ...
            for (var k = 0; k < faceIndices.length; k++) {
                average += vertexHeights[planetMesh.geometry.faces[faceIndex][faceIndices[k]]];
            }
        }
        ...
        for (var j = 0; j < faces.length; j++) {
            faceIndex = faces[j];
            setFaceColor(heightPercent, planetMesh.geometry.faces[faceIndex]);
        }
    }
...
function generateHeightTilePlanet(planet, planetSize, minHeight, maxHeight) {
    ...
    for (var i = 0; i < planet.polygonGroups.length; i++) {
        ...
        for (var j = 0; j < faces.length; j++) {
            var faceIndex = faces[j];
            for (var k = 0; k < faceIndices.length; k++) {
                average += planetMesh.geometry.vertices[planetMesh.geometry.faces[faceIndex][faceIndices[k]]].length() / planetSize;
            }
        }
...
function generateFlatHeightTilePlanet(planet, planetSize, minHeight, maxHeight) {
    ...
    for (var i = 0; i < planet.polygonGroups.length; i++) {
        ...
        var polygonMesh = createPolygonMesh(planetMesh, planet.polygonGroups[i].vertices, average);
        for (var j = 0; j < polygonMesh.geometry.faces.length; j++) {
            setFaceColor(heightPercent, polygonMesh.geometry.faces[j]);
        }
...
var terrainType = {
    mountainTop:  { threshold: 0.95, color: 0xC8C8C8 },
    darkMountain: { threshold: 0.85, color: 0x735B5B },
    mountain:     { threshold: 0.80, color: 0x826868 },
...
function createPolygonMesh(planetMesh, polygon, height) {
    var geometry = new THREE.Geometry();
    var material = new THREE.MeshPhongMaterial({ side: THREE.FrontSide, shading: THREE.FlatShading, vertexColors: THREE.VertexColors });

    var vertices = planetMesh.geometry.vertices;
